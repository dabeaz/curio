...
# All rights reserved.
#
# Standin for the standard socket library.  The entire contents of
# stdlib socket are made available here.  However, the socket class is
# replaced by an async compatible version.
# Certain blocking operations are also replaced by versions safe to use
# in async.
...
async def create_connection(*args, **kwargs):
    sock = await workers.run_in_thread(
        _socket.create_connection, *args, **kwargs)
...
async def gethostbyname_ex(*args, **kwargs):
    return await workers.run_in_thread(
        _socket.gethostbyname_ex, *args, **kwargs)
